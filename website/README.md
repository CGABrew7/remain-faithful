...
|---|---|---|
| `BACKEND_URL` | Yes | Base URL of the Go backend API |
...
4. Add environment variables in the Vercel dashboard:
   - `BACKEND_URL` → your backend URL (e.g. `https://api.remainfaithful.com`)
   - `NEXT_PUBLIC_SITE_URL` → `https://remainfaithful.com`
...
|---|---|---|
| `/api/donate` | POST | Proxies to backend `/donations/create-checkout-session`, returns `{ url }` for Stripe redirect |
| `/api/contact` | POST | Proxies to backend `/contact` |
