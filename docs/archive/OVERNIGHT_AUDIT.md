> **OBSOLETE — historical June 2026 working paper. Not current product documentation.**
>
> This file is a snapshot of a June 2026 design review. **Tier 3 cloud OCR and the backend `/classify` path (Anthropic) were REMOVED.** They are not part of the shipping product.
>
> **Current product:**
> - Classification is **on-device only** (Vision OCR, SensitiveContentAnalysis, local keyword/domain/hash checks). Screenshots, OCR text, and raw screen content never leave the device.
> - Partners may receive a short **system-generated summary** in addition to category, timestamp, and severity — never screenshots, OCR text, or raw screen content.
> - There is **no Anthropic classify path** and no server-side OCR.
>
> Findings below (PrivacyInfo, JWT issuer, `/classify` rate limits, “disclose Anthropic OCR,” etc.) describe a design that no longer exists or have since been superseded. Do not treat this document as a live architecture, privacy disclosure, or App Store promise.

# Overnight Audit — Remain Faithful
Date: 2026-06-13
Status: **OBSOLETE** (archived June 2026 working paper)
